...
// Graceful shutdown — это процесс корректного завершения работы приложения,
// при котором оно завершает текущие операции и освобождает ресурсы
// перед окончательным выключением
func (a *App) Stop() {
	...
	// - ждёт завершения обработки текущих запросов
	// - возвращает управление
	// Затем в main() нам нужно в правильный момент вызвать этот метод.
	// Для этого нам нужно научить программу перехватывать сигналы
	// SIGINT и SIGTERM от ОС (т.е. понимать, когда от ОС пришла команда остановки работы).
